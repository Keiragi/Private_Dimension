...
# 終了条件 + 設置処理
  execute if score $step.current PrivateDim.PlotOp = $step PrivateDim.PlotOp run function s.private_dimension:place/my_plot/get_plot

# 余りが0なら60度回転 そうでなければそのままで前進
  execute if score $step.mod PrivateDim.PlotOp matches 0 rotated ~60 0 positioned ^ ^ ^4285000 run return run function s.private_dimension:place/place_coordinate/vertex
